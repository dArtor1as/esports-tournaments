...
import { Dota2SimulatorService } from '../../src/match-simulators/simulators/dota2-simulator.service';
...
// Список реальних команд для Dota 2
const dota2TeamsData = [
...
export async function seedDota2(prisma: PrismaClient, admin: User, game: Game) {
  console.log(' Починаємо генерацію екосистеми Dota 2...');
  ...
  const dota2Simulator = new Dota2SimulatorService();
  const dummyCache = {
    get: async () => null,
    set: async () => {},
    del: async () => {},
  ...
  console.log(`Створення ${dota2TeamsData.length} команд для Dota 2...`);

  // 1. СТВОРЕННЯ КОМАНД І ГРАВЦІВ
  for (let i = 0; i < dota2TeamsData.length; i++) {
    const tData = dota2TeamsData[i];
    let totalElo = 0;
    const rosterData: {
      playerId: string;
      role: RosterRole;
    ...
    const dota2Roles = ['POS_1', 'POS_2', 'POS_3', 'POS_4'];

    // 4 Гравці
    for (let j = 1; j <= 4; j++) {
      const pRating = getRandomRating(2500, 3200);
      const playerRole = dota2Roles[j - 1];
      totalElo += pRating;
  ...
  console.log(' Генерація 3 минулих турнірів Dota 2 (для статистики)...');
  ...
  for (let season = 1; season <= 3; season++) {
    ...
    const pastTournament = await prisma.tournament.create({
      data: {
        title: `Dota 2 Season ${2023 + season}`,
        gameId: game.id,
        tier: 1,
    ...
    while (currentRoundTeams.length > 1) {
      ...
      for (let i = 0; i < currentRoundTeams.length; i += 2) {
        ...
        const matchResult = dota2Simulator.simulateSeries(
          teamAInput,
          teamBInput,
          expectedProbA,
          3, // BO3
    ...
    console.log(`Обробка статистики та Elo для сезону ${season} Dota 2...`);
